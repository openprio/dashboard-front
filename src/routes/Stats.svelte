<script lang="ts">
  ...
  async function loadDailyStats(roadRegulatorId) {
    ...
    try {
      let response = await fetch(
        `https://dashboard-api.openprio.nl/stats?agg=daily&road_regulator=${roadRegulatorId}&filter_operating_hours=true`,
        {
          headers: {
            Authorization: "Bearer " + token,
          },
  ...
  async function loadMonthlyStats(roadRegulatorId) {
    ...
    try {
      let response = await fetch(
        `https://dashboard-api.openprio.nl/stats?agg=monthly&road_regulator=${roadRegulatorId}&filter_operating_hours=true`,
        {
          headers: {
            Authorization: "Bearer " + token,
          },
  ...
  $effect(() => {
    if (selectedRoadRegulator) {
      Promise.all([
        loadMonthlyStats(selectedRoadRegulator),
        loadDailyStats(selectedRoadRegulator),
      ]);
    }
  });
...
<div class="m-4 flex flex-row">
  ...
  <div class="m-2 w-80">
    ...
    <p class="mb-4 mt-4 leading-relaxed">
      Voor deze statistiek worden alle kruisingen meegenomen waarvoor gedurende
      een dag ten minste 1 SSM GRANTED is ontvangen. In de statistiek wordt per
      kruispuntpassage weergegeven of er OpenPrio, SRM, SSM PROCESSING en SSM
      GRANTED berichten gezien zijn.
    </p>
    ...
    <p class="mb-8 mt-4 leading-relaxed">
      ...
      moment dat alle verkeerslichten in Delft operationeel zijn en ritten die
      eindigen voor het moment dat de eerste VRI’s worden uitgezet (ma-vr 07-21,
      za 09-21, zo 10-21). Voor Delft zijn alle passages van tram 19 uit de
      statistiek gehaald.
    </p>
  </div>
</div>
