<script lang="ts">
    ...
    async function loadMonthlyStats(roadRegulatorId) {
        ...
        try {
            ...
            let openprio = result.map(dataPoint => { return {"x": dataPoint.month, "y": ((dataPoint.count_openprio_received /  dataPoint.journey_intersection_passes) * 100.0).toFixed(1)}});
            let srm = result.map(dataPoint => { return {"x": dataPoint.month, "y": ((dataPoint.count_srm_sent /  dataPoint.journey_intersection_passes) * 100.0).toFixed(1)}});
            let processing = result.map(dataPoint => { return {"x": dataPoint.month, "y": ((dataPoint.count_ssm_processing /  dataPoint.journey_intersection_passes) * 100.0).toFixed(1)}});
            let granted = result.map(dataPoint => { return {"x": dataPoint.month, "y": ((dataPoint.count_ssm_granted/  dataPoint.journey_intersection_passes) * 100.0).toFixed(1)}});
    ...
    const options_month_graph = {
        ...
        scales: {
            ...
            x: {
                ...
                suggestedMin: new Date(new Date().getFullYear() - 1, new Date().getMonth(), 1).toISOString(),
                adapters: {
                    date: {
                        locale: nl
                    }
...
<div class="flex flex-row m-4">
    ...
    <div class="w-80 m-2">
        ...
        <p class="leading-relaxed mt-4 mb-8">
            In deze statistiek worden alleen ritten opgenomen die starten na het moment dat alle
            verkeerslichten in Delft operationeel zijn en ritten die eindigen voor het moment dat de
            eerste VRI’s worden uitgezet (ma-vr 07-21, za 09-21, zo 10-21). Voor Delft zijn alle passages
            van tram 19 uit de statistiek gehaald.        
        </p>
