<script lang="ts">
    ...
    const columnDefs = [
        ...
        colHelp.accessor('first_openprio_message', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'Eerste OpenPrio <150m')
            ),
            cell: ( cell ) => (
        ...
        colHelp.accessor('first_srm_new', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'Eerste SRM new')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
        ...
        colHelp.accessor('first_ssm_requested', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'SSM requested')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
        ...
        colHelp.accessor('first_ssm_processing', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'SSM processing')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
        ...
        colHelp.accessor('first_ssm_granted', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'SSM granted')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
        ...
        colHelp.accessor('first_srm_cancellation', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'SRM cancellation')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
        ...
        colHelp.accessor('last_openprio_message', { 
            header: () => (
                renderSnippet(defaultHeaderTitle, 'Laatste OpenPrio')
            ),
            cell: ( cell ) => (
                renderSnippet(errorIfEmptyCell, extract_timestamp(cell.getValue()))
            )
