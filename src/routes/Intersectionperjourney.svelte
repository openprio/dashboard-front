<script lang="ts">
    ...
    async function loadData(operationDate, roadRegulator, intersectionId) {
        try {
        let response = await fetch(`https://dashboard-api.openprio.nl/intersection_stats_per_journey?road_regulator=${roadRegulator}&operation_date=${operationDate}&intersection=${intersectionId}&line_planning_number=${line_planning_number}&direction=${direction}`);
        if (!response.ok) {
